...
# список данных введенных пользователем
note_info = { }

# Ввод имени пользователя
user = input('Введите имя пользователя:\n')
note_info.update(username = user)
print('Пользователь: ',note_info['username'])

# создание заголовков
title = []
while True: # задаем цикл
    add_title = input('Можете ввести несколько заголовков или оставьте поле пустым \n') # Просим ползователя ввести данные
    if add_title == '' or add_title == ' ':# Проверяем данные пользователя где пустое значение остановит цикл
        ...
        title.append(add_title)  # Добавление значения в список через метод .append
note_info.update(titles = title)
print('Ваши заголовки: ',note_info['titles'])
...
note_info.update(content = key_list)
print('Ваши заметки: ',note_info['content'])
...
note_info.update(status_note = status)
...
note_info.update(created_date = created_date)
print('Дата создания заметки: ',note_info['created_date'])
...
while True:
    try:
        issue_date = input('Введите дату дедлайна (в формате xx-xx-xxxx)\n') # ввод даты пользователя  формате дд-мм-гггг
        issue_date = datetime.strptime(issue_date, '%d-%m-%Y')
        break
    except:
        print('Введен невеный формат даты, попробуйте еще раз ')
note_info.update(issue_date = issue_date)
print('Дата окончания заметки: ', note_info['issue_date'])
